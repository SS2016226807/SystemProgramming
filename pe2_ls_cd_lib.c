...
void r_directory(char dir_name[])
{
    DIR *dir_ptr;
    struct dirent *dirtp;
    ...
    if((dir_ptr = opendir(dir_name) )== NULL)
    {
        fprintf(stderr, "ls -R : cannot open %s\n", dir_name);
        printf("\n");
        return;
    }
//    printf("Directory name : %s\n", dirtp->d_name);
    ...
    while((dirtp = readdir(dir_ptr)) != NULL)
    {
        ...
        if(stat(dirtp->d_name, &stat_info) == -1)
        {
            perror("stat file");
            continue;
        }
        if(S_ISDIR(stat_info.st_mode))
        {
           ...
           dir_names[count] = dirtp->d_name;
           count += 1;
          //  printf(" %s \t", dirtp->d_name);
           // r_directory(dirtp->d_name);
        }
        else
        {
            printf(" %s \t", dirtp->d_name);
